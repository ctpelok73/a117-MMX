				       ...
				       MUINT32 u4GroupId)
{
    ...
    MY_LOGD("3A get exif");
    IHal3A* p3AHal = IHal3A::createInstance(IHal3A::E_Camera_1,MtkCamUtils::DevMetaInfo::queryHalSensorDev(0),"PreviewFeature");
...
{
	  MBOOL ret = true;
    halUTILITYBase* UtilityObj = halUTILITYBase::createInstance();
              ...
              , MUINT32     &u4JpegSize)
{
	  MBOOL ret = true;
    halUTILITYBase* UtilityObj = halUTILITYBase::createInstance();                                  	
    ret=UtilityObj->createMAVJpegImg(pMPImageInfo
                                  ,Srcbuf
                                  ,u4SrcWidth,u4SrcHeight
                                  ,BasObj->mShotNum
                                  ,u4JpegSize,mpParamsMgr->getInt(CameraParameters::KEY_JPEG_QUALITY));
...
handlePanoImgCallBack(MVOID* const puJpegBuf, int u4SrcWidth, int u4SrcHeight)
{
    ...
    IImageBufferAllocator::ImgParam imgParam((u4SrcWidth * u4SrcHeight * 2), 0);
...
//    MUINT32 u4JpegSize;
//    
//    BasObj->createJpegImg((u4SrcWidth * u4SrcHeight * 2),-1,(MVOID*)trueJpegBuf,0
//                          ,u4SrcWidth,u4SrcHeight
//                          ,jpegBuf->getBufSize(),(MINT32)jpegBuf->getIonFd(),(MVOID*)jpegBuf->getBufVA(0),(MUINT32)jpegBuf->getPhyAddr(0)
//                          ,u4JpegSize);
//                          	
//    //save final image
...
handleMAVImgCallBack(MVOID* const puJpegBuf, int u4SrcWidth, int u4SrcHeight)
{
    ...
    pMPImageInfo[0].imageBuf = (char*)jpegBuf->getBufVA(0);
    pMPImageInfo[0].imageSize = u4JpegSize ;
    pMPImageInfo[0].sourceType = SOURCE_TYPE_BUF;
    #if 0
    char sourceFiles[80];
    sprintf(sourceFiles, "%s.jpg", "/sdcard/MPO0");
    dumpBufToFile((char *) sourceFiles, (MUINT8 *)jpegBuf->getBufVA(0), u4JpegSize);
    #endif
   
    for (MUINT8 i = BasObj->mShotNum; i > 0 ; i--) {
        MY_LOGD("MAV JPG: %d Adr 0x%x", i, (MUINT32)pMPImageInfo[i].imageBuf);
