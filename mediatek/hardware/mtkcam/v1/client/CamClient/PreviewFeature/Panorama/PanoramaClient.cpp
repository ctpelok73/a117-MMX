...
init(int bufwidth,int bufheight)
{
    ...
    imgParam.imgSize = MOTION_MAX_IN_WIDTH * MOTION_MAX_IN_HEIGHT * 3;
    if(!(allocMem(imgParam,mpMotionBuffer)))
    {
    ...
    imgParam.imgSize = initBufSize;
    if(!(allocMem(imgParam,mpPanoramaWorkingBuf)))
    {
    ...
    mAutoramaInitInData.WorkingBufAddr = (MUINT32)mpPanoramaWorkingBuf->getBufVA(0);
    ...
    mMotionInitInfo.WorkingBuffAddr = (MUINT32)mpMotionBuffer->getBufVA(0);
