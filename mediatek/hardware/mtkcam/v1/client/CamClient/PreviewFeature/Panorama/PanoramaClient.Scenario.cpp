...
mHalCamFeatureProc(MVOID * bufadr, int32_t& mvX, int32_t& mvY, int32_t& dir, MBOOL& isShot)
{
    ...
    CreateMotionSrc(bufadr, mPanoramaFrameWidth, mPanoramaFrameHeight, (MVOID*)mpMotionBuffer->getBufVA(0));
    err = ISShot((MVOID*)mpMotionBuffer->getBufVA(0), &PanoramaResult, isShot);
    ...
    if (isShot)
    {
        ...
        if (mPanoramaFrameIdx < PanoramaNum)
        {
            ...
            MUINT8 *pbufOut = (MUINT8 *) (mpframeBuffer->getBufVA(0) + ((mPanoramaFrameWidth * mPanoramaFrameHeight * 3 / 2) * mPanoramaFrameIdx));
