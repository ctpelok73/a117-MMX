...
onCreate()
{
    ...
    {
#define BYTE2PIXEL(PLANE, BYTE, FORMAT)                             \
    (BYTE << 3) / Format::queryPlaneBitsPerPixel(FORMAT, PLANE)     \
        ...
        mvImgBufInfo[i]->sizeInBytes   = (eImgFmt_JPEG == getImgFormat())
                                            ? mvImgBufInfo[i]->stridesInBytes  // size in bytes.
                                            : imgHeightInPixels * mvImgBufInfo[i]->stridesInBytes;
        ...
        {   // check  ROI(x,y) + ROI(w,h) <= heap stride(w,h)
            ...
            MUINT32 const planeStartXInPixels   = BYTE2PIXEL(i, planeStartXInBytes, getImgFormat());
            MUINT32 const planeStartYInPixels   = BYTE2PIXEL(i, planeStartYInBytes, getImgFormat());
            MINT32  const planeStridesInPixels  = BYTE2PIXEL(i, mspImgBufHeap->getBufStridesInBytes(i), getImgFormat());
    ...
    for (MUINT32 i = 0; i < getPlaneCount(); ++i)
    {
        MY_LOGD_IF(mspImgBufHeap->getLogCond(), "plane(%d) buf stride(%d), bufSize(%d)", i, getBufStridesInBytes(i), getBufSizeInBytes(i));
    }
    //
    return  MTRUE;
}
