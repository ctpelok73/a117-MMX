		...
		if (c != prefix_length) {
			...
			if (c < 0) {
			    ...
			    if (errno==ETXTBSY || errno==ENODEV) {
			        ...
			        LOGMUX(LOG_ERR, "write_frame check error casue errno=%d \n", errno);
                            }
			}
            pthread_mutex_unlock(&write_frame_lock);
			return 0;
 ...
 * Return:   0 on success (OK-response), -1 otherwise
 */
static int chatCmux(int	serial_device_fd, char * cmd, int to)
{
	LOGMUX(LOG_NOTICE, "Enter");
	/* Note by LS: buf[] is used to store the AT cmd; it may not use so large buf-size (e.g., 1024) */
	/* Currently, the maximum size passed to chat is start_muxer(): gsm_command[100] */
	unsigned char buf[1024];
	int wrote = 0;
	...
	SYSCHECK(wrote = write(serial_device_fd, cmd, strlen(cmd)));
#endif  /* REVERSE_MTK_CHANGE */
	LOGMUX(LOG_NOTICE, "Wrote %d bytes", wrote);
	...
	while (1) {
        ...
        #endif
		sel = select(serial_device_fd + 1, &rfds, NULL, NULL, &timeout);
        LOGMUX(LOG_NOTICE, "select sel:%d, checkMuxFlag:%d", sel, checkMuxFlag);
		if (sel < 0) {
			if (errno == EINTR) {
				continue;
			} else {
			...
			if (FD_ISSET(serial_device_fd, &rfds)) {
				...
				len = read(serial_device_fd, buf, sizeof(buf));
				SYSCHECK(len);
				LOGMUX(LOG_NOTICE, "Read %d bytes from serial device", len);
				syslogdump("<s ", buf, len);
				//errno = 0;
				if (memstr((char *)buf, len, "OK")) {
                    if(checkMuxFlag == 1) {
					...
					LOGMUX(LOG_NOTICE, "Received OK, it is old modem, so sleep(1)");
					return 0;
				} else if (memstr((char *)buf, len, "+CMUX: READY")) {
				    LOGMUX(LOG_NOTICE, "Received CMUX: READY, it is new modem, start to init control channel");
                    checkMuxFlag = 1;
				    continue;
                } else if (memstr((char *)buf, len, "ERROR")) {
					LOGMUX(LOG_WARNING, "Received ERROR");
...
int start_muxer(Serial *serial)
{
	...
	SYSCHECK(chatCmux(serial->fd, gsm_command, 3));
