...
void requestSetPreferredNetworkType(void * data, size_t datalen, RIL_Token t)
{
    ...
    ****************************************/
    ...
    //4G Preferred (4G, 3G/2G) item (for MMDSDC mode)
    case NT_LTE_GSM_WCDMA_MMDC:
    case NT_LTE_GSM_WCDMA_MMDC_RAT_ONLY_TYPE:
        rat = 5;
        rat1= 4;
        break;
    // 3G or 2G Preferred (3G/2G -> 4G)(for MMDSDC mode)
    case NT_GSM_WCDMA_LTE_MMDC:
    case NT_GSM_WCDMA_LTE_MMDC_RAT_ONLY_TYPE:
        rat = 5;
        rat1= 2;
        break;
    // 4G or 2G (for MMDSDC mode)
    case NT_LTE_GSM_MMDC:
    case NT_LTE_GSM_MMDC_RAT_ONLY_TYPE:
        rat = 3;
        rat1= 0;
        break;
    //For LTE --End
    ...
    if(rat >= 0)
    {
        //[ALPS1259204] for EPSD confirm
        #ifdef MTK_LTE_DC_SUPPORT
            LOGD("send AT+EPSD=3,2");
            err = asprintf(&cmd,  "AT+EPSD=3,2");
            err = at_send_command(cmd, NULL, getRILChannelCtxFromToken(t));
        #endif
...
void requestGetPreferredNetworkType(void * data, size_t datalen, RIL_Token t)
{
    ...
    //for LTE -- START
        ...
        // 3G or 2G (only TDD project in CSFB mode)
        return_type = NT_GSM_WCDMA_LTE;

    } else if(nt_type == 5 && prefer_type == 4) {
        //4G Preferred (4G, 3G/2G) item (only for MMDSDC mode)
        return_type = NT_LTE_GSM_WCDMA_MMDC;
    } else if(nt_type == 5 && prefer_type == 2) {
        //3G or 2G item (only for MMDSDC mode)
        return_type = NT_GSM_WCDMA_LTE_MMDC;
    //for LTE -- END
...
void onNetworkStateChanged(char *urc, const RILId rid)
{
    ...
    {    
        /* Note: There might not be full +CREG URC info when screen off     
            Full URC format for 2/3G   : +CREG: <stat>, <lac>, <cid>, <Act>, <cause> 
            Full URC format for Lte(4G): +CREG: <stat>, <lac>, <cid>, <Act>, <cause_type>, <cause> */
...
extern int rilNwMain(int request, void *data, size_t datalen, RIL_Token t)
{
    ...
    case RIL_REQUEST_GET_NEIGHBORING_CELL_IDS:
        LOGD("handle RIL_REQUEST_GET_NEIGHBORING_CELL_IDS");	  			
