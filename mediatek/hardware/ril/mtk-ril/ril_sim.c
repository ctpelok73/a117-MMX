...
void setSimStatusChanged(RILId rid)
{
    ...
    // for MMDC modem, unlock PIN is send to protocol 1, 
    // we needs to send sim status changed to ril2 to notify framework refresh sim status 
    RIL_onUnsolicitedResponse (
        RIL_UNSOL_RESPONSE_SIM_STATUS_CHANGED,
        NULL, 0, MTK_RIL_SOCKET_2);
#endif
        ...
        if (result->pin1 == 0) {
            ...
            if ( USIM_READY == sim_status || SIM_READY == sim_status ) {
                ...
                if(rid == MTK_RIL_SOCKET_1) {
                    setRadioState(RADIO_STATE_SIM_READY, MTK_RIL_SOCKET_2);
                }
            #endif
            }
            simStatusChanged++;
...
#ifdef MTK_LTE_DC_SUPPORT
    // for MMDC modem, always ask sim status from protocol 1
    err = at_send_command_singleline("AT+CPIN?", "+CPIN:", &p_response, getChannelCtxbyProxy(MTK_RIL_SOCKET_1));
#else
