...
static void initializeCallback(void *param)
{
    ...
    // Query current modem type
    triggerCCCIIoctlEx(CCCI_IOC_GET_MD_TYPE, &mdType);
    setActiveModemType(mdType);
    LOGD("[LteMSH][initCallback] Set ril.active.md = %d", mdType);
    #endif
    #endif
    ...
    if (0 != nRadioState) {
        requestSimInsertStatus(rid);
    } else if(MTK_SHARE_MODEM_SUPPORT > 1) {
        requestSimReset(rid);
    }

    flightModeBoot();

    if(MTK_SHARE_MODEM_SUPPORT == 1 && nRadioState == 0){
        requestSimInsertStatus(rid);
    }
...
#ifdef __ATCI_CHANNEL_SUPPORT__
    "/dev/radio/atci1-md2",
#endif    
#ifdef  MTK_GEMINI
...
static void *mainLoop(void *param)
{
	...
	while (s_main_loop) {
        ...
        if (performSWSimSwitchIfNecessary(sim3G)) {
            initialCidTable();
            if (isDualTalkMode()) {
                RLOGI("DualTalk mode and default switch status: %d", s_is3GSwitched);
            } else {
                RLOGI("Is 3G Switch now: [%d], 3G SIM mapped for framework: [SIM%d]", s_is3GSwitched, sim3G);
                char* value = NULL;
...
#endif  /* MTK_RIL */
{
    ...
    if (*pState != newState || s_closed > 0) {
        if(isLteDCSupport()){
            char network_mode[PROPERTY_VALUE_MAX] = { 0 };
            char data_status[PROPERTY_VALUE_MAX] = { 0 };
            property_get("gsm.mmdc.network.mode", network_mode, "0");
            property_get("ril.mobile.data", data_status, "0");
            LOGI("gsm.mmdc.network.mode: %s, ril.mobile.data: %s\n", network_mode, data_status);
            
            // no needs to send sim1 radio state changed for radio on if network mode is gsm only or data is closed
            // because mmdc modem request sim command always send to p1, so radio state of sim1 will be update by sim2 request.
            // so needs to special handing that do not send radio state changed in this case
            // properties value define in RilConstants.java
            // int NETWORK_MODE_WCDMA_PREF     = 0; /* GSM/WCDMA (WCDMA preferred) */
            // int NETWORK_MODE_GSM_ONLY       = 1; /* GSM only */
            if((atoi(data_status) != 0 && atoi(network_mode) != 1) || rid != MTK_RIL_SOCKET_1) {
                *pState = newState;
                assert(0);
                pthread_cond_broadcast(&s_state_cond); 
            } else {
                if((newState == RADIO_STATE_OFF || newState == RADIO_STATE_UNAVAILABLE || newState == RADIO_STATE_SIM_NOT_READY)) {
                    *pState = newState;
                    assert(0);
                    pthread_cond_broadcast(&s_state_cond); 
                }
