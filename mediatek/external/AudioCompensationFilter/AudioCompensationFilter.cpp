...
bool AudioCompensationFilter::Start(void)
{
    ...
    if (mpTempBuf != NULL)
    {
        ALOGD("mpTempBuf != NULL");
        memset(mpTempBuf, 0, mtmpSize);
    }

    if (mpBloudHandle != NULL)
    {
        ALOGD("mpBloudHandle=0x%x, mpWorkingBuf=0x%x, sizeof(mInitParam)=%d, &mInitParam=0x%x, mpBloudHandle->bEnableOLA=%d", mpBloudHandle, mpWorkingBuf, sizeof(mInitParam), &mInitParam, mpBloudHandle->bEnableOLA);
        ALOGD("A Channel_Num [%d]",mInitParam.Channel_Num);
        ALOGD("A Sample_Rate [%d]",mInitParam.Sample_Rate);
        ALOGD("A Filter_Mode [%d]",mInitParam.Filter_Mode);
        ALOGD("A Loudness_Mode [%d]",mInitParam.Loudness_Mode);
...
#endif
        ALOGD("B Channel_Num [%d]",mInitParam.Channel_Num);
        ALOGD("B Sample_Rate [%d]",mInitParam.Sample_Rate);
        ALOGD("B Filter_Mode [%d]",mInitParam.Filter_Mode);
        ALOGD("B Loudness_Mode [%d]",mInitParam.Loudness_Mode);
        mActive = true;
    }
    ALOGD("AudioCompensationFilter::Start() End");
    return true;
}
//mpBloudHandle->Open can be only called during init
//no need to called before every process called or after mInitParam modified
