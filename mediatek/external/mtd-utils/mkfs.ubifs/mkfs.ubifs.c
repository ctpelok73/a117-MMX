...
static unsigned source_path_len = 0;
...
static int get_options(int argc, char**argv)
{
	...
	while (1) {
		...
		case 'x':
			...
			else if (strcmp(optarg, "lz4k") == 0)
				c->default_compr = UBIFS_COMPR_LZ4K;			
	...
	if (1) {
		...
		if (!root) {
			printf("Fixstats (-F) option cannot requested on null direcroty!\n");
			exit(0);
		}

		len = strlen(root);
		if ((len >= 5) && (!strcmp(root + len - 5, "data/"))) {
			source_path_len = len - 5;
		} else if ((len >= 7) && (!strcmp(root + len - 7, "system/"))) {
			source_path_len = len - 7;
		} else if ((len >= 6) && (!strcmp(root + len - 6, "secro/"))) {	
			source_path_len = len - 6;
		} else {
			printf("Fixstats (-F) option requested but filesystem is not data or android!\n");
			exit(0);
		}
	}
...
//MTK start
int vol_lebs = 0;
int ubifs_calc_min_idx_lebs()
{
        int idx_lebs, idx_size;
	int max_idx_node_sz =  ubifs_idx_node_sz(c, c->fanout);
	int idx_leb_size = c->leb_size - max_idx_node_sz;

        idx_size = c->old_idx_sz + max_idx_node_sz;
         ...
         */
        idx_lebs = (idx_size + idx_leb_size - 1) / idx_leb_size;
        /*
...
long long ubifs_calc_available(int min_idx_lebs)
{
        int subtract_lebs = 0;
        long long available, main_bytes;

	main_bytes = (long long)(c->main_lebs+vol_lebs-c->leb_cnt) * c->leb_size;
        available = main_bytes - c->lst.total_used;
...
{
	int min_idx_lebs, rsvd_idx_lebs, lebs, extra_need; //MTK
        long long  data_need; //MTK
...
//MTK start
	vol_lebs = (c->max_leb_cnt*10+10)/11;
	/* First budget index space */
        min_idx_lebs = ubifs_calc_min_idx_lebs();
	//printf("min_idx_lebs %d\n", min_idx_lebs);
	if (min_idx_lebs > c->lst.idx_lebs)
                rsvd_idx_lebs = min_idx_lebs - c->lst.idx_lebs;
        else
                rsvd_idx_lebs = 0;

	lebs = c->lst.empty_lebs - 1 + c->max_leb_cnt/1.1+1 - c->leb_cnt;

        data_need = ubifs_calc_available(min_idx_lebs);
        //printf("data_need %d\n", data_need);
	rsvd_idx_lebs += ((data_need+c->leb_size-1)/c->leb_size);
	//printf("lebs %u, rsvd_idx_lebs %u\n", lebs, rsvd_idx_lebs);
	//printf("max_lebs %u\n", (c->max_leb_cnt*10/11));
        if (rsvd_idx_lebs > lebs) {
		extra_need = 6 + vol_lebs + (rsvd_idx_lebs-lebs);
                //printf("leb_cnt too low(%u(%uMB) needed)", extra_need, (extra_need+3)/4);
                return err_msg("leb_cnt too low(%u(%uMB) needed)", extra_need, (extra_need+3)/4);
	}
//MTK end
	if (verbose) {
		printf("\tsuper lebs:   %d\n", UBIFS_SB_LEBS);
