...
int anim_fb_init(void)
{
    ...
    LOG_ANIM("[charging_animation: %s %d]vinfo.xres  = %d, vinfo.yres = %d, vinfo.xres_virtual =%d, vinfo.bits_per_pixel = %d, fb_size =%d, fb_addr = %d,back_fb_addr=%d\n"
            ,__FUNCTION__, __LINE__, vinfo.xres,vinfo.yres, vinfo.xres_virtual, vinfo.bits_per_pixel,fb_size,( int)fb_addr, (int)back_fb_addr);
