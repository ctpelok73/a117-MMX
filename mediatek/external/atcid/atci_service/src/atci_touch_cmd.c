...
void touch_down(int dev, int x, int y)
{
    ...
    char touch_event_4[40];                                         //ABS_MT_POSITION_X
    char touch_event_5[40];                                         //ABS_MT_POSITION_Y
    ...
    sprintf(touch_event_4, "sendevent /dev/input/event3 3 53 %d", x);
    sprintf(touch_event_5, "sendevent /dev/input/event3 3 54 %d", y);
...
int touch_cmd_parser(char *cmdline, TP_POINT points[], int *point_num)
{
    ...
    {
        if(num == 0)
            valid_para = sscanf(pcmd, "%d,%d,%d%s", &points[num].action, &points[num].x, &points[num].y, legency_cmd);
        else
            valid_para = sscanf(legency_cmd, "%*[^=]=%d,%d,%d%s", &points[num].action, &points[num].x, &points[num].y, legency_cmd);
