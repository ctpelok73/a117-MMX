...
#ifdef USING_LOCAL_SOCKET
{
    ...
    while (!g_ThreadExitSocket)
    {
        ...
        {
            MTK_NFC_MSG_T socket_closed_msg;
            ...
            socket_closed_msg.type = MTK_NFC_EXIT_REQ;
            socket_closed_msg.length = 0;
            NFCD("nfc_socket_thread, send msg to service (MTK_NFC_EXIT_REQ)\n");               
            mtk_nfc_sys_msg_send(MTK_NFC_TASKID_SERVICE, &socket_closed_msg);  
        }
    }
