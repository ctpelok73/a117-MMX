...
//chenmeiai added  for test MHL s
static jboolean hdmi_enable_test(JNIEnv *env, jobject clazz)
{
    /*bool ret = false;
    //ret = ioctl(fd, MTK_HDMI_FACTORY_MODE_ENABLE, 1);
#if defined (MTK_HDMI_SUPPORT)
    ret = hdmi_ioctl(MTK_HDMI_FACTORY_MODE_ENABLE, 1);
#endif
    ...
    factory = ioctl(fd, MTK_HDMI_FACTORY_MODE_ENABLE, 1);
	if(!factory)
	{
		ret = true;
	}
...
//chenmeiai added  for test MHL e
...
static jboolean getSupported(JNIEnv* env, jobject clazz, jint aformat) {
    bool ret = false;
#if defined (MTK_HDMI_SUPPORT)
    int value = 0;
    if (aformat == 0) {
        value = HDMI_RESIZE_SUPPORT;
    }
    if (hdmi_ioctl(MTK_HDMI_GET_CAPABILITY, (int)&value) == false) {
        ret = false;
    } else {
        if (value > 0) {
            ret = true;
...
#endif
    ALOGI("[HDMI] JNI com.mediatek.hdmi.HDMINative.getSupported(%d)\n", ret);
    return ret;
}
...
//    { "GetDeviceStatus","(ZZ)Z", 	(void*)GetDeviceStatus},
    ...
    { "getSupportedNative",     "(I)Z",       (void*)getSupported},
	{ "hdmi_enable_test",	"()Z",        (void*)hdmi_enable_test},  //chenmeiai added
};
