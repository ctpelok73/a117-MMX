...
public final class MtkHdmiManagerService extends IMtkHdmiManager.Stub {
    ...
    // This LID is only used for 8389 talbet project to read hdcp key from nvram, ignore on other projects
    ...
    private static final int HDMI_VIDEO_SCALE_DEFAULT = -1;
    ...
    private boolean setVideoResolutionImpl(int resolution, int scale) {
        ...
        synchronized (mHdmiNative) {
            return mHdmiNative.setVideoConfig(param);
        }
    }
    ...
    public boolean setVideoScale(int scale) {
        ...
        if (ret) {
            ...
            int finalResolution = mHdmiVideoResolution >= HdmiDef.AUTO ? (mHdmiVideoResolution - HdmiDef.AUTO)
                    : mHdmiVideoResolution;
    ...
    private boolean supportScale() {
        synchronized (mHdmiNative) {
            return mHdmiNative.getSupported(0);
        }
    }
