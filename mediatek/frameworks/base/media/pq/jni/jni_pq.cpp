...
Java_com_mediatek_pq_PictureQuality_setGalleryNormalMode(JNIEnv* env, jobject thiz)
{
    ...
    if (drvID < 0)
    {
        //XLOGE("[PQ JNI] setGalleryNormalMode, open /proc/mtk_mdp_color failed!! ");
        drvID = open("/dev/mtk_mdp_cmdq", O_RDONLY, 0);
    }
    if (drvID < 0)
    {
        //XLOGE("[PQ JNI] setGalleryNormalMode, open /proc/mtk_mdp_cmdq failed!! ");
        drvID = open("/dev/mtk_disp", O_RDONLY, 0);
    }
    if (drvID < 0)
    {
