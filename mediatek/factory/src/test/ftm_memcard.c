...
static void mcard_update_info(struct mcard *mc, char *info)
{
    ...
    if (mc->avail && !mc->mounted) {
        ...
        //For EMMC project without multi-storage support, 
        // try the following sequence: mmcblk0p1 -> mmcblk0 -> emmc@fat
        #ifdef MTK_EMMC_SUPPORT
            len = sprintf(mc->dev_path, "/dev/block/mmcblk%dp1\0", 1);
        #else
            len = sprintf(mc->dev_path, "/dev/block/mmcblk%dp1\0", 0);
        #endif

        fd = open(mc->dev_path, O_RDONLY);
        #ifdef MTK_EMMC_SUPPORT
        if (fd < 0) {
            len = sprintf(mc->dev_path, "/dev/block/mmcblk%d\0", 1);
            fd = open(mc->dev_path, O_RDONLY);
            if (fd < 0) {
                len = sprintf(mc->dev_path, "/emmc@fat\0");
                fd = open(mc->dev_path, O_RDONLY);
            }
        }
        #endif
