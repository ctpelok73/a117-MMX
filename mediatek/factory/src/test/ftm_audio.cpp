...
#else
static void mAudio_receiver_playtone(struct mAudio *mc, void *priv)
{
#if defined(WAVE_PLAYBACK)
    memset(mc->file_name, 0, MAX_FILE_NAME_SIZE);
...
static void mAudio_receiver_stoptone(struct mAudio *mc)
{
    pthread_join(mc->hHeadsetThread, NULL);
}
    ...
    do
    {
        ...
        switch (chosen)
        {
            case ITEM_RINGTONE:
                ...
                else if (privChosen == ITEM_MIC3)
                {
                    usleep(3000);
                    PhoneMic_Receiver_Loopback(MIC3_OFF);  // disable Receiver MIC3 loopback
                }
                else if (privChosen == ITEM_MIC4)
                {
                    usleep(3000);
                    PhoneMic_Receiver_Loopback(MIC4_OFF);  // disable Receiver MIC4 loopback
                }
            ...
            case ITEM_MIC3:
                ...
                usleep(3000);
                PhoneMic_Receiver_Loopback(MIC3_ON);  // enable Receiver MIC3 loopback
                privChosen = ITEM_MIC3;
                exit = false;
                break;
            case ITEM_MIC4:
                if (privChosen == ITEM_MIC4)
                {
                    break;
                }
                else if (privChosen == ITEM_MIC1)
                ...
                usleep(3000);
                PhoneMic_Receiver_Loopback(MIC4_ON);  // enable Receiver MIC4 loopback
                privChosen = ITEM_MIC4;
                exit = false;
                break;
            case ITEM_PASS:
            case ITEM_FAIL:
