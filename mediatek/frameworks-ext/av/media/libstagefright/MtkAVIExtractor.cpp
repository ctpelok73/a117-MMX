...
status_t MtkAVIExtractor::parseSTRF(MtkAVIOffT pos, MtkAVIOffT end, sp<MtkAVISource> source) {
    ...
    if (source->mIsVideo) {
		...
		{
		    //uint8_t* extraData = source->strfData->data();
			if(source->strfData->data() != NULL
				&&source->strfData->size() != 0){
			    ...
			    LOGE("aHeader extra data is null");
				//return ERROR_UNSUPPORTED;
			}
		}
		if(!strcmp(mime, MEDIA_MIMETYPE_AUDIO_MS_ADPCM) 
...
status_t MtkAVISource::read(
    ...
    *out = NULL;
	...
	if(mMP3Header < 0 && afterseek && (dts<seekTimeUs && seekTimeUs-dts>100000))
	{
		NeedRefinePos = true;	
		mCurrentDTS = dts;
		LOGI("seekTimeUs-dts>100000!!!! dts=%lld,NeedRefinePos=%d",dts,NeedRefinePos);
	}
