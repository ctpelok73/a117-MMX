...
AudioMTKMixer::TimeStretchBufferProvider::TimeStretchBufferProvider(int framecount) : AudioBufferProvider(),
        mTrackBufferProvider(NULL), mTimeStretchHandle(NULL), mOutBuffer(NULL), mBuffer(NULL),mOutRemain(0)
{
	if(framecount >0)
	{
...
void AudioMTKMixer::TimeStretchBufferProvider::releaseBuffer(AudioBufferProvider::Buffer *pBuffer) {
	...
	ALOGV("TimeStretchBufferProvider::releaseBuffer()");
	...
	ALOGV("release pBuffer-> raw %x, pBuffer->frameCount %d",pBuffer->raw ,pBuffer->frameCount  );
}
...
int64_t pts) {
	...
	short* OutBufferPtr;
	status_t res;
	ALOGV("TimeStretchBufferProvider::getNextBuffer()");
	...
	if(mOutRemain !=0)
	{
		...
		ALOGV("TimeStretchBufferProvider::getNextBuffer() directly return");
		pBuffer->frameCount = mOutRemain;
		pBuffer->raw = mOutBuffer;
		return OK;
	}
	...
	ALOGD("mOutframecount%d, pBuffer->frameCount %d",mOutframecount,pBuffer->frameCount);
	...
	//ALOGD(" pBuffer->frameCount = pBuffer->frameCount -mOutRemain %d",pBuffer->frameCount);
...
void AudioMixer::unprepareTrackForTimeStretch(track_t* pTrack, int trackName) {
    ...
    if (pTrack->timestretchBufferProvider != NULL) {
        ...
        ALOGV(" deleting old TimeStretch");
	if(pTrack->timestretchBufferProvider->mOutRemain !=0)
	{
		ALOGV("unprepareTrackForTimeStretch clean internal buffer");
...
void AudioMTKMixer::track_t::updateDRCParam(void)
{
    ...
    if (mpDRCObj) {
        ...
        AUDIO_ACF_CUSTOM_PARAM_STRUCT DRCParam;
        GetAudioCompFltCustParamFromNV(AUDIO_COMP_FLT_AUDIO, &DRCParam);
        
        mpDRCObj->SetParameter(BLOUD_PAR_SET_PREVIEW_PARAM, (void *)&DRCParam);
...
void AudioMTKMixer::track_t::setDRCHandler(audio_devices_t device, uint32_t bufferSize, uint32_t sampleRate)
{
    ...
    if ( (true==mUIDRCEnable) &&
        ...
        if (mpDRCObj == NULL) {
            ...
            mpDRCObj->SetParameter(BLOUD_PAR_SET_USE_DEFAULT_PARAM_SUB, (void *)NULL);
            mpDRCObj->SetParameter(BLOUD_PAR_SET_SAMPLE_RATE, (void *)sampleRate);
            mpDRCObj->SetParameter(BLOUD_PAR_SET_CHANNEL_NUMBER, (void *)BLOUD_HD_STEREO);
            mpDRCObj->SetParameter(BLOUD_PAR_SET_PCM_FORMAT, (void *)BLOUD_IN_Q1P31_OUT_Q1P31);
            mpDRCObj->SetParameter(BLOUD_PAR_SET_FILTER_TYPE, (void *)AUDIO_COMP_FLT_AUDIO);
