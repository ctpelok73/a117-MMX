...
void HWComposer::setWaitFramebufferTarget(int32_t id) {
    if (id >= VIRTUAL_DISPLAY_ID_BASE &&
        !hwcHasApiVersion(mHwc, HWC_DEVICE_API_VERSION_1_3))
        return;
    ...
    Mutex::Autolock _l(mFramebufferTargetSync[id].waitLock);
    mFramebufferTargetSync[id].needWait = true;
}
