...
#define SW_WATCHDOG_THRESHOLD   2000
...
static int aee_ioctl_wdt_kick(int value)
{
    ...
    if (fd < 0)	
    {
        XLOGD("SurfaceFlingerWatchdog:ERROR: open %s failed.\n",AE_WDT_DEVICE_PATH);
        return 1;
    }
    else
    {
        ...
        {
            XLOGD("SurfaceFlingerWatchdog :ERROR: aee wdt kick powerkey ioctl failed.\n");
            close (fd);
            return 1;
        }
    }
...
bool SFWatchDog::threadLoop() {
    ...
    //else
    //{	
    //	XLOGV("[SF-WD] mUpdateSreen not update!!!!!: %d", mUpdateSreen);
