...
static struct i2c_board_info __initdata kd_lens_dev={ I2C_BOARD_INFO("OV8835AF", 0x1c)};//LINE <> <DATE20130626> <add sensor:ov8835,ov12830> wupingzhou
...
inline static int Register_OV8835AF_CharDrv(void)
{
    ...
    actuator_class = class_create(THIS_MODULE, "actuatordrv6");		//LINE <> <20130523> <add sensor:IMX179> lishengli
...
static int OV8835AF_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    OV8835AFDB("[OV8835AF] OV8835AF_i2c_probe\n");

    /* Kirby: add new-style driver { */
    g_pstOV8835AF_I2Cclient = client;

	g_pstOV8835AF_I2Cclient->addr =  OV8835AF_VCM_WRITE_ID; //LINE <> <DATE20130626> <add sensor:ov8835,ov12830> wupingzhou  
...
static struct platform_driver g_stOV8835AF_Driver = {
    ...
    .driver		= {
        .name	= "lens_actuator6",		//LINE <> <20130523> <add sensor:IMX179> lishengli
        .owner	= THIS_MODULE,
    }
};
...
static int __init OV8835AF_i2C_init(void)
{
    ...
    platform_device_register(&actuator_dev6);		//LINE <> <20130523> <add sensor:IMX179> lishengli
