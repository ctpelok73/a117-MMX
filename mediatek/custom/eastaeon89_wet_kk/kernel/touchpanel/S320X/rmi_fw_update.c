...
static struct bin_attribute dev_attr_data = {
	.attr = {
		.name = "data",
		.mode = (S_IRUGO /* | S_IWUGO  change by panchenjun for cts test failed 20130722*/),
	},
	.size = 0,
...
static struct device_attribute attrs[] = {
	__ATTR(doreflash, S_IRUGO,//S_IWUGO, //change by panchenjun for cts test failed 20130722
			synaptics_rmi4_show_error,
			fwu_sysfs_do_reflash_store),
	__ATTR(writeconfig, S_IRUGO, //S_IWUGO,//change by panchenjun for cts test failed 20130722
			synaptics_rmi4_show_error,
			fwu_sysfs_write_config_store),
	__ATTR(readconfig, S_IRUGO, //S_IWUGO,//change by panchenjun for cts test failed 20130722
			synaptics_rmi4_show_error,
			fwu_sysfs_read_config_store),
	__ATTR(configarea, S_IRUGO,//S_IWUGO,//change by panchenjun for cts test failed 20130722
			synaptics_rmi4_show_error,
			fwu_sysfs_config_area_store),
	__ATTR(imagesize, S_IRUGO,//S_IWUGO,//change by panchenjun for cts test failed 20130722
			synaptics_rmi4_show_error,
...
static unsigned int ConvertConfigID(const unsigned char* ptr) 
{
	int retval;
	retval = (unsigned int)(ptr[3]-0x30)+
		     (unsigned int)(ptr[2]-0x30)*10+
		     (unsigned int)(ptr[1]-0x30)*100+
		     (unsigned int)(ptr[0]-0x30)*1000;
	return retval;
}
int synaptics_fw_updater(unsigned char *fw_data)
{
	...
	////////////////////////////////////////////////////////////////////////////
	//compare the device ID with the image ID to see whether update or not
	//if your had designed the config_id by yourself,you should re-implement the compare function.
    ////////////////////////////////////////////////////////////////////////////
    ret = fwu->fn_ptr->read(fwu->rmi4_data,fwu->f34_fd.ctrl_base_addr,config_id,4);
	device_configID = ConvertConfigID(config_id);
	image_configID = ConvertConfigID(fw_data+0xb100);

	if(image_configID > device_configID){		//add by lishengli 20130516 TEST
	//if(0){
...
//LINE<JIRA_ID><DATE20130523><BUG_INFO>zenghaihui
int synaptics_get_fw_version(void)
{
    int ret;
    unsigned char config_id[4];
    int device_configID = 0;
...
//	fwu->config_area = UI_CONFIG_AREA;
    ...
    if (!properties_kobj_fwupdate) {
			dev_err("%s: Failed to create sysfs directory\n",
					__func__);
			goto err_sysfs_dir;
	}
