...
#include "SynaImage_YJ.h"
#include "SynaImage_BIEL.h"		//add by lishengli 20130913
...
static struct i2c_board_info __initdata i2c_tpd={ I2C_BOARD_INFO(DRIVER_NAME, (0x72))};		//add by lishengli 20130514
...
//	queue_delayed_work(det_workqueue,&det_work,msecs_to_jiffies(EXP_FN_DET_INTERVAL));
    ...
    if(product_id)
    {
        ...
        else if( 0 == memcmp(product_id, "YJ", 2))
        {
            //default YJ TP
            printk("[S3202]: %s YJ TP ready to upgrade \n", __func__);
            synaptics_fw_updater(synaImage_YJ);
        }
        else if( 0 == memcmp(product_id, "BIEL", 4))		//add by lishengli 20130913
        {
            //default BIEL TP
...
//begin   add by lishengli 20130527
/*
