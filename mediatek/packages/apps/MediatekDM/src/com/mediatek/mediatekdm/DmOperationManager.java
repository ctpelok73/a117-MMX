...
public class DmOperationManager {
    ...
    private class OperationScanner implements Runnable {
        ...
        public OperationScanner(IOperationScannerHandler handler, String folderPath, DmOperationManager manager) {
            mHandler = handler;
            mFolderPath = folderPath;
            mManager = manager;
        }
                    ...
                    if (fileList != null && fileList.length > 0) {
                        ...
                        for (String filePath : fileList) {
                            ...
                            try {
                                ...
                                mExecutorService.execute(new OperationRemover(operation, mOperationFolderPath));
                                e.printStackTrace();
                            }
                        }
                    }
        ...
        synchronized (DmOperationManager.class) {
            if (sInstance == null) {
                sInstance = new DmOperationManager(
                        Path.getPathInData(DmApplication.getInstance(), Path.DM_OPERATION_FOLDER));
            }
            return sInstance;
        }
    }
    ...
    public synchronized void enqueue(DmOperation operation, boolean writeToFS) {
        Log.d(TAG, "+enqueue()");
        if (!mQueue.contains(operation) && !(mCurrent != null && mCurrent.getId() == operation.getId())) {
            ...
            Log.w(TAG, "Operation " + operation.getId() + " is already in queue. Ignore this enqueue action.");
        }
        Log.d(TAG, "-enqueue()");
    }
    ...
    /**
     * Recover the current operation. This method will not decrease the retry counter of current operation. Client should
     * invoke this method when non-fatal error recovers before the time-out is emitted.
     */
    ...
    /**
     * Retry the current operation. This method will decrease the retry counter of current operation. Client should invoke
     * this method after the current operation has timed out.
     */
    ...
    /**
     * Equivalent to invoking TriggerResult triggerNow(DmOperation operation, boolean writeToFS) with writeToFS equals to
     * false.
     * 
     * @param operation
     * @return
    ...
    /**
     * This method can only be called when there is no operation running. It will cut in line and set the operation passed in
     * as current running operation. NOTE: the operation inserted will ignore skip mark.
     * 
     * @param operation the operation to be triggered
     * @param writeToFS whether write the operation to file system
     * @return
     */
    ...
    private boolean prepareFolder() {
        ...
        if (!dirFolder.exists()) {
            ...
            if (dirFolder.mkdir()) {
                ...
                Log.e(TAG, "Failed to create " + dirFolder.getAbsolutePath() + ", failed to save operation data");
                Log.d(TAG, "-prepareFolder()");
                return false;
            }
        }
        ...
        if (!operationFolder.exists()) {
            ...
            if (!operationFolder.mkdirs()) {
                Log.e(TAG, "Failed to create " + mOperationFolderPath + ", failed to save operation data");
                Log.d(TAG, "-prepareFolder()");
                return false;
            }
        }
