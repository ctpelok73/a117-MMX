...
public class MdmEngine {
    ...
    /**
     * MDM engine constructor. There can only be one instance of MdmEngine or the constructor will raise a MdmException.
     * Default logger will be used.
     * 
     * @param context Android context.
     * @param mmiFactory MMI factory.
     * @param plFactory PL factory.
     * @throws MdmException
     */
    ...
    /**
     * Destroy the engine. After active engine instance is destroyed, user can <b>new</b> another instance of engine.
     * @throws MdmException 
     */
    ...
    /**
     * Start the engine. This method must be called before any other MdmEngine method. If an exception is thrown,then no
     * other MdmEngine method should be called except for {@link MdmEngine#destroy()}.
     * 
     * @throws MdmException
     */
    ...
    /**
     * Check whether there are active sessions and if the queue is empty. There can at most one active session at any time.
     * 
    ...
    /*
     * Default logger implementation for MdmLog. If no logger is provided in constructor, this default logger will be used.
     * This default logger will output log to Android log.
     */
    ...
    /**
     * Register as a session state observer. A session state observer is notified whenever a session state changes. The
     * SessionStateObserver.notify() method is called once on each change in the session state. There can be multiple session
     * state observers.
     * 
     * @param observer A session state observer.
     */
    ...
    /**
     * Unregister a session state observer. The observer is removed from the list of session state observers.
     * 
     * @param observer Session observer to unregister.
     */
    ...
    /**
     * Register a handler for download retry. The handler is notified when a non fatal error occurs while downloading an
     * object, just before the Engine attempts to restore the connection. The handler may approve the retry attempt or return
     * an error code which will cause the session to be aborted, and the code will be passed as lastError to all
     * session-state observers.
     * 
     * @param handler Download Retry observer or null to unregister.
     */
     ...
     */
    private void notifySessionStateObservers(SessionType type, SessionState state, int lastError) throws MdmException {
    ...
    public void removePendingTrigger(SessionInitiator initiator) {
        ...
        for (TriggerRecord tr : mTriggerQueue) {
            mLogger.logMsg(MdmLogLevel.DEBUG, "Check pending trigger " + tr.sessionInitiator.getId());
            if (tr.sessionInitiator == initiator) {
                recordToRemove = tr;
                break;
            }
        }
        mLogger.logMsg(MdmLogLevel.DEBUG, "Remove pending trigger " + recordToRemove.sessionInitiator.getId());
    ...
    /**
     * Get Session-Enable mode. The setting for this mode determines which sessions sessions that have been triggered may be
    ...
    /**
     * Set Session-Enable mode. The setting for this mode determines which sessions sessions that have been triggered may be
     * started by the Engine. For example, you can allow only server-initiated sessions. The initial value is
     * MdmSessionEnableMode.allowAll.
     * 
     * @param mode A MDMSessionEnableMode value.
     * @throws MdmException INVALID_CALL if vDirect Mobile is not initialized yet.
     */
    ...
    private void triggerSession(TriggerRecord tr) throws MdmException {
        MdmEngine.getLogger().logMsg(MdmLogLevel.DEBUG, "[ENGINE] Trigger queued session with type " + tr.triggerType);
        switch (tr.triggerType) {
            case NIA_DM:
                triggerNIADmSessionInternal(
                        (byte[]) tr.parameters.get("Message"),
                        tr.sessionInitiator,
                        (NIAMsgHandler) tr.parameters.get("NIAMsgHandler"));
                break;
            case DM:
                triggerDMSessionInternal(
                        (String) tr.parameters.get("Account"),
            ...
            case DL:
                ...
                triggerDLSessionInternal(
                        (uri == NULL_OBJECT ? null : ((String) uri)),
                        (DownloadPromptHandler) tr.parameters.get("PromptHandler"),
                        tr.sessionInitiator);
                break;
            case DM_GENERIC_ALERT:
                triggerGenericAlertSessionInternal(
                        (String) tr.parameters.get("Correlator"),
                        (String) tr.parameters.get("Format"),
                        (String) tr.parameters.get("Type"),
                        (String) tr.parameters.get("Mark"),
                        (String) tr.parameters.get("Source"),
                        (String) tr.parameters.get("Target"),
                        (String) tr.parameters.get("Data"),
                        (String) tr.parameters.get("Account"),
                        tr.sessionInitiator);
                break;
            default:
    ...
    /**
     * Trigger a notification-initiated DM session. The DM client should call this method after receiving a
     * "General Notification Initiated Session Alert" (NIA). To receive notification when a notification-initiated (DM)
     * session state changes, register as a session observer. To unregister as a session state observer, call
     * unregisterSessionStateObserver. If the engine is not idle, then this trigger will be queued and will be triggered when
     * the engine is idle.
     * 
     * @param message Notification message content.
     * @param initiator Session initiator that will be passed as a parameter upon session state notifications.
     * @param handler Handler of a parsed NIA message. The handler will be called to handled the parsed message before a
     *            connection with the server is established.
     * @throws MdmException
     */
    public synchronized void triggerNIADmSession(byte[] message, SessionInitiator initiator, NIAMsgHandler handler)
            throws MdmException {
        mLogger.logMsg(MdmLogLevel.DEBUG, "[ENGINE] +triggerNIADmSession()");
        if (mSessionEnableMode != MdmSessionEnableMode.allowAll) {
            mLogger.logMsg(MdmLogLevel.WARNING, "Server initiated session is not allowed according to configuration.");
    ...
    private void triggerNIADmSessionInternal(byte[] message, SessionInitiator initiator, NIAMsgHandler handler)
            throws MdmException {
     ...
     * 
     * @param message
     * @param initiator Not used in native layer.
     * @param handler Not used in native layer.
     * @return
     */
    private native int triggerNIADmSessionN(byte[] message, SessionInitiator initiator, NIAMsgHandler handler);
    ...
    /**
     * Trigger a DM session. To receive notification when a DM session state changes, register as a session observer. To
     * unregister as a session state observer, call unregisterSessionStateObserver.
     * 
     * @param account The URI of the DM Account node in the DM tree (null for default DM account).
     * @param genericAlertType Generic alert type as defined by OMA. For example:
     *            org.openmobilealliance.dm.firmwareupdate.devicerequest is the generic alert type used for device initiated
     *            firmware update.
     * @param message Vendor-specific message content. CURRENTLY NOT SUPPORTED and must be null.
     * @param initiator Session initiator that will be passed as a parameter upon session state notifications.
     * @throws MdmException with a MdmError error code
     */
    public synchronized void triggerDMSession(
            String account,
            String genericAlertType,
            byte[] message,
    ...
    private void triggerDMSessionInternal(String account,
            String genericAlertType,
            byte[] message,
            SessionInitiator initiator) throws MdmException {
     ...
     * 
     * @param uri Uri to download descriptor. If this parameter is null, then DL will resume the last download.
    ...
    private void triggerDLSessionInternal(String uri, DownloadPromptHandler promptHandler, SessionInitiator initiator)
            throws MdmException {
    ...
    /**
     * Notify that server notification has been handled. Called by the NIA message handler to notify that server notification
     * has been handled.
     * 
     * @throws MdmException
     */
    ...
    /**
     * Trigger a Report session (DM Session). To receive notification when a Report session state changes, register a session
     * observer. To unregister as a session state observer, call unregisterSessionStateObserver.
     * 
     * @param uriPath The URI of a node in the DM tree.
     * @param reasonCode Result code. Used when the alert is an asynchronous response to an Exec command.
     * @param account The URI of the DM Account node in the DM tree. (null for default DM account).
     * @param genericAlertType Generic alert type as defined by OMA. For example:
     *            org.openmobilealliance.dm.firmwareupdate.download is the generic alert type used in response to the
     *            completion of a Download operation.
     * @param correlator Correlator previously sent by server. Used when the alert is an asynchronous response to an Exec
     *            command.
     * @param initiator Session initiator that will be passed as a parameter upon session state notifications.
     * @throws MdmException
     */
    public synchronized void triggerReportSession(String uriPath,
            int reasonCode,
            String account,
            String genericAlertType,
            String correlator,
    ...
    public synchronized void triggerReportSession(
            String correlator,
            String format,
            String type,
            String mark,
        ...
        triggerGenericAlertSession(correlator, format, type, mark, source, target, data, account, initiator);
        mLogger.logMsg(MdmLogLevel.DEBUG, "[ENGINE] -triggerReportSession()");
    }

    /**
     * This is the most general trigger for CI generic alert session. It only do basic sanity check of the parameters.
     * 
     * @param correlator
     * @param format Cannot be null if data is not null. Only "int", "text/plain" and "xml" are supported.
     ...
     * @param data
     * @param account The URI of the DM Account node in the DM tree. (null for default DM account).
     * @param initiator Cannot be null.
     * @throws MdmException
     */
    public synchronized void triggerGenericAlertSession(
            String correlator,
            String format,
            String type,
            String mark,
        ...
        if (!isIdleN()) {
            mLogger.logMsg(MdmLogLevel.DEBUG, "Engine is not idle, enqueue the trigger.");
            TriggerRecord tr = new TriggerRecord(TriggerRecord.TriggerType.DM_GENERIC_ALERT, initiator);
            ...
            triggerGenericAlertSessionInternal(correlator, format, type, mark, source, target, data, account, initiator);
        }
        mLogger.logMsg(MdmLogLevel.DEBUG, "[ENGINE] -triggerGenericAlertSession()");
    }

    private synchronized void triggerGenericAlertSessionInternal(
            String correlator,
            String format,
            String type,
            String mark,
        ...
        if (data != null &&
                !(format != null && (format.equals("int") || format.equals("text/plain") || format.equals("xml")))) {
            throw new MdmException(MdmError.BAD_INPUT);
        }

        clearSessionActions();
        if (0 != triggerGenericAlertSessionN(
                correlator,
                format,
                type,
                mark,
    ...
    private native int triggerGenericAlertSessionN(
            String correlator,
            String format,
            String type,
            String mark,
     ...
     * 
     * @param actions Action mask. Bits enabled in this mask will be set.
     */
     ...
     * 
     * @param actions Action mask. Bits enabled in this mask will be cleared.
     */
    ...
    /*
     * MmiObserver implementation which notify native engine thread with MMI response from user. The heavy-lifting work is
     * done in native land.
     */
