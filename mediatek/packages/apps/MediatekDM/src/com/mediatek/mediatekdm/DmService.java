...
public class DmService extends Service {
    ...
    private class DmServiceHandler extends Handler implements IOperationStateObserver {
        ...
        // This heart beat is an experience safe value for reserving a invocation time window for triggerNow().
...
//            super(getMainLooper());
            ...
            switch (msg.what) {
                case IServiceMessage.MSG_OPERATION_PROCESS_NEXT:
                    ...
                    if (result == TriggerResult.BUSY) {
                        ...
                        // Clear redundant MSG_OPERATION_PROCESS_NEXT in queue. When the current operation is finished,
                    ...
                    if (operation.getProperty(KEY.TYPE).equals(Type.TYPE_SI)) {
                        ...
                        if (!accepted) {
                            Log.e(TAG.SERVICE, "No suitable handler to process operation " + operation);
                        }
                    }
                    break;
                ...
                case IServiceMessage.MSG_OPERATION_RECOVER_CURRENT:
                    ...
                    if (operation.getProperty(KEY.TYPE).equals(Type.TYPE_SI)) {
                        ...
                        for (IDmComponent component : mComponents) {
                            if (component.dispatchOperationAction(OperationAction.RECOVER, operation) == DispatchResult.ACCEPT) {
                                break;
                            }
                        }
                ...
                case IServiceMessage.MSG_CHECK_NETWORK:
                    Log.i(TAG.SERVICE, "Receive MSG_CHECK_NETWORK after " + (NETWORK_CHECK_INTERVAL / 1000) + "s.");
        ...
        public void notify(State state, State previousState, Object extra) {
            if (state == State.IDLE) {
                ...
                Log.i(TAG.SERVICE, "In recover, Send MSG_CHECK_NETWORK after " + (NETWORK_CHECK_INTERVAL / 1000) + "s.");
                    ...
                    switch (uiMode) {
                        ...
                        case 2:
                            ...
                            if (lastResponse == InteractionResponse.INVALID) {
                                ...
                                setNiaAlertTimeout(NiaSessionManager.DEFAULT_NOTIFICATION_VISIBLE_TIMEOUT,
                                        NotificationInteractionType.TYPE_NOTIFICATION_VISIBLE);
                            }
                            mDmController.triggerNiaDmSession(message);
                            break;
                        case 3:
                            ...
                            if (lastResponse == InteractionResponse.INVALID) {
                                ...
                                setNiaAlertTimeout(NiaSessionManager.DEFAULT_NOTIFICATION_INTERACT_TIMEOUT,
                                        NotificationInteractionType.TYPE_NOTIFICATION_INTERACT);
    ...
    private static class DmSessionStateObserver implements SessionStateObserver {
        ...
        public void notify(SessionType type, SessionState state, int lastError, SessionInitiator initiator) {
            ...
            for (IDmComponent component : DmApplication.getInstance().getComponents()) {
                sessionHandler = component.dispatchSessionStateChange(
                        type,
                        state,
                        lastError,
                        initiator,
    ...
    public void onCreate() {
        ...
        if (CMCCComponent.getRegisteredSimId(this) == -1) {
            Log.e(TAG.SERVICE, "Sim is not registered!");
            mFatalErrorOccurred = true;
            Process.killProcessQuiet(Process.myPid());
            return;
        }
        ...
        Log.e(TAG.SERVICE, "Service Handler's looper belongs to thread " + mServiceHandler.getLooper().getThread().getId());
...
//            Utilities.removeDirectoryRecursively(getFilesDir());
        ...
        DmOperationManager.getInstance().registerObserver((IOperationStateObserver) mServiceHandler);
    }
    ...
    public void onDestroy() {
        ...
        DmOperationManager.getInstance().unregisterObserver((IOperationStateObserver) mServiceHandler);
        try {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (action.equals(IntentAction.DM_WAP_PUSH)) {
            ...
            if (type != null && type.equals(DmConst.IntentType.DM_NIA)) {
                int result = DmDataConnection.getInstance(this).enableDataConnectivityForWapPushMessage();
            ...
            if (intent.getBooleanExtra("response", false)) {
                ...
                mDmController.triggerNiaDmSession(mOperationManager.current().getByteArrayProperty(KEY.NIA));
            } else {
            ...
            if (DmFeatureSwitch.CMCC_SPECIFIC) {
                // CMCC DM 3.0 specification demands that DM client should reply negative if timeout occurred.
                mOperationManager.current().addUIResponse(
                        new InteractionResponse(InteractionType.CONFIRMATION, InteractionResponse.NEGATIVE, null));
                mAlertConfirm.cancel();
            } else {
                // OMA DM specification demands that DM client should use DR in server's request as the default response.
                mOperationManager.current().addUIResponse(
            ...
            if (intent.getBooleanExtra("found", false)) {
                ...
                if (!mOperationManager.isBusy() && !mOperationManager.hasNext()) {
                    Log.d(TAG.SERVICE, "No pending operation found and service is idle, schedule exit message.");
                    cutHP();
                }
                return START_NOT_STICKY;
            }
        ...
        if (DmConfig.getInstance().useMobileDataOnly()) {
                ...
                if (mOperationManager.isInRecovery()) {
                    mServiceHandler.sendMessage(
                            mServiceHandler.obtainMessage(
                                    IServiceMessage.MSG_OPERATION_RECOVER_CURRENT,
                                    0,
                                    0,
            ...
            Log.i(TAG.SERVICE, "Dump queue A which belongs to thread " + mServiceHandler.getLooper().getThread().getId());
            mServiceHandler.getLooper().getQueue().dumpMessageQueue();
            Log.i(TAG.SERVICE, "Dump queue B which belongs to thread " + getMainLooper().getThread().getId());
    ...
    /*
     * This method is used to control the life cycle of DmService, but it is only applied to the onStart situation. FIXME Not
     * complete. Do NOT rely on it.
     */
    ...
    private void initDmController() {
        mDmController = new DmController(this, new DmSessionStateObserver(this), mNiaManager, mNiaManager,
                new MmiFactory() {
                    ...
                    public MmiProgress createProgress(int total) {
                        return new MmiProgress() {
                            public void update(int current, int total) {
                                ...
                                if (operation != null) {
                                    for (IDmComponent component : mComponents) {
                                        component.dispatchMmiProgressUpdate(operation, current, total);
                                    }
                                }
                            }
                        };
    ...
    private void prepareDmTreeFile() {
        ...
        File dataTree = new File(Path.getPathInData(this, Path.DM_TREE_FILE));
