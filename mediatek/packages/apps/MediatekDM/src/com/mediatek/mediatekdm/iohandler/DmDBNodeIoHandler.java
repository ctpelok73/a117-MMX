...
public abstract class DmDBNodeIoHandler implements NodeIoHandler, ICacheable {
            ...
            if (mUri.getPath().contains(getItem()[i])) {
                if ((String) mMap.get(getItem()[i]) != null) {
                    if (specificHandlingForRead(getItem()[i]) != null) {
                        ...
                        Log.d(TAG.NODEIOHANDLER, "Special read result = "
                                + recordToRead);
                    }
                    if (recordToRead.length() == 0) {
                        ...
                        Log.d(TAG.NODEIOHANDLER,
                                "Normal read, sqlString = " + sqlString);
    ...
    public void write(int arg0, byte[] arg1, int arg2) throws MdmException {
        ...
        if (mRecordToWrite.length() == arg2) {
            ...
            if (uriPath != null) {
                ...
                if (indexOfSlash == -1) {
                    Log.e("DmService", "index of / of mUri is null");
                    return;
                }
                uriPathName = uriPath.substring(indexOfSlash + 1);
            } else {
                Log.e("DmService", "mUri.getPath is null!");
                return;
            }
    ...
    protected final String getMccMnc() {
        if (DmConfig.getInstance().useSmsReg()) {
            if (mMccMnc == null) {
                int simSlot = CMCCComponent.getRegisteredSimId(mContext);
                if (simSlot != -1) {
                    mMccMnc = PlatformManager.getInstance().getSimOperator(simSlot);
                    Log.d("DmService", "getSimOperator(" + simSlot + "): " + mMccMnc);
                } else {
                    Log.e("DmService", "No sim card registered!");
                    /* should not reach here, dummy string */
                    return "22222";
                }
            }

            Log.d("DmService", "mMccMnc is " + mMccMnc);
            return mMccMnc;
        } else {
            return null;
        }
