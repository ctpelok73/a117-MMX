...
/**
 * Entry for Client initiated firmware update. It's only a UI for DmClient. See 5.1.5.4.2 in China Mobile Device Management
...
public class DmEntry extends PreferenceActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        currentVersionPreference.setTitle(getResources().getString(R.string.current_version) + " " + Build.DISPLAY);
        currentVersionPreference.setSummary(getResources().getString(R.string.release_date) + " " + buildDate);
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        ...
        if (DmConfig.getInstance().useSmsReg() && CMCCComponent.getRegisteredSimId(mContext) == -1) {
            showDialog(DIALOG_SIM_NOT_REGISTERED);
        ...
        switch (id) {
            ...
            case DIALOG_DISCARD_AND_START_NEW:
                return DialogFactory
                        ...
                        .create();
            case DIALOG_SIM_NOT_REGISTERED:
                return DialogFactory
                        .newAlert(this)
                        .setTitle(R.string.system_update)
                        .setMessage(R.string.sim_not_register)
                        .setPositiveButton(R.string.start_new_session, // TODO use a separate string for this
                                new DialogInterface.OnClickListener() {
                                    public void onClick(DialogInterface dialog, int whichButton) {
                                        // do nothing
                                    }
    ...
    private void updateUI() {
        ...
        if (fumoState == DmFumoState.DOWNLOAD_PAUSED) {
            mUpdatePreference.setSummary(getResources().getString(R.string.download_status) + " " + dlVersion);
            ...
            mUpdatePreference.setSummary(
                    getResources().getString(R.string.download_status) + " " + mCurrentProgress + "%");
        } else {
            mUpdatePreference.setSummary("");
        }
        ...
        if (mOperationManager.isBusy()) {
            ...
            boolean isFumoDL =
                    op.getProperty(KEY.TYPE).equals(Type.TYPE_DL) && op.getBooleanProperty(KEY.FUMO_TAG, false);
            if (!isFumoDL) {
        ...
        if (fumoState == DmFumoState.DOWNLOAD_COMPLETE) {
            ...
            mInstallPreference.setSummary(getResources().getString(R.string.install_version) + " " + dlVersion);
        } else {
            mInstallPreference.setEnabled(false);
            mInstallPreference.setSummary("");
        }
    ...
    private IDmFumoStateObserver mFumoObserver = new IDmFumoStateObserver() {
        @Override
        public void notify(int newState, int previousState, final DmOperation operation, Object extra) {
    ...
    private static final int DIALOG_SIM_NOT_REGISTERED = 2;
}
