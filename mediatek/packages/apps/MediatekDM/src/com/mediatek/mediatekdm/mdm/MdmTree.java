...
public class MdmTree {
    ...
    private static class TreeManagerAgent {
        ...
        public String buildTndsString(String subtreeUri) throws MdmException {
            final StringBuilder sb = new StringBuilder("<MgmtTree xmlns='syncml:dmddf1.2'><VerDTD>1.2</VerDTD>");
            sb.append("<Man>")
              .append(getStringValue("./DevInfo/Man"))
              .append("</Man><Mod>")
              .append(getStringValue("./DevInfo/Mod"))
              .append("</Mod>");
            walkTree(subtreeUri, new TreeIterator() {
                @Override
                public int access(String uri, int mode) throws MdmException {
                    MdmEngine.getLogger().logMsg(MdmLogLevel.DEBUG, "buildTndsString.access: " + uri + ", " + mode);
                    if (mode == TreeIterator.MODE_BEFORE || mode == TreeIterator.MODE_LEAF) {
                        String[] parts = splitNameAndPath(uri);
                        sb.append("<Node><NodeName>")
                          .append(parts[1])
                          .append("</NodeName><Path>")
                          .append(parts[0])
                          .append("</Path>");
                    }
                    ...
                    if (mode == TreeIterator.MODE_LEAF) {
                        ...
                        if (format.equals("bin")) {
                            ...
                            MdmEngine.getLogger().logMsg(MdmLogLevel.DEBUG, "Bin value length is " + length);
    ...
    public String[] listChildren(String nodeUri) throws MdmException {
        try {
            ...
            if (children == null || children.length() == 0) {
                MdmEngine.getLogger().logMsg(MdmLogLevel.DEBUG, "[TREE] Children is null or 0 children.");
                return new String[0];
            } else {
                MdmEngine.getLogger().logMsg(MdmLogLevel.DEBUG, "[TREE] listChildren length " + children.length());
