...
public class DmConfig {
    ...
    private DmConfig(Context context) {
        mContext = context;
        String f = Path.getPathInData(mContext, Path.DM_CONFIG_FILE);
        mConfigFile = (new File(f).isFile()) ? f : Path.getPathInSystem(Path.DM_CONFIG_FILE);
        InputStream is = null;
        try {
    ...
    public void configure() {
        try {
            ...
            if (mParamTable.containsKey(KEY_DM_HEX_SID) && mParamTable.getProperty(KEY_DM_HEX_SID).equals("true")) {
            ...
            if (mParamTable.containsKey(KEY_DM_WBXML) && mParamTable.getProperty(KEY_DM_WBXML).equals("true")) {
            ...
            if (mParamTable.containsKey(KEY_SEQUENTIAL_NONCE) && mParamTable.getProperty(KEY_SEQUENTIAL_NONCE).equals("true")) {
    ...
    public boolean restartOnLock() {
        if (mParamTable.containsKey(KEY_RESTART_ON_LOCK) && mParamTable.getProperty(KEY_RESTART_ON_LOCK).equals("true")) {
            return true;
        } else {
            return false;
        }
    ...
    public boolean getCollectSetMsgPermission() {
        if (mParamTable.containsKey(KEY_COLLECT_SET_MSG_PERMISSION) && mParamTable.getProperty(KEY_COLLECT_SET_MSG_PERMISSION).equals("true")) {
            return true;
        } else {
            return false;
        }
