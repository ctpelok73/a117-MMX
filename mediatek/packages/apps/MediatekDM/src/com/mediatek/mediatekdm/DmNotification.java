        ...
        switch (type) {
            ...
            case NotificationInteractionType.TYPE_NOTIFICATION_INTERACT:
                notification = new Notification(
                        R.drawable.stat_download_info,
                        mService.getString(mNotificationText),
                        System.currentTimeMillis());
                notification.flags = Notification.FLAG_AUTO_CANCEL;
                notification.setLatestEventInfo(
                        mService,
                        mService.getText(R.string.app_name),
                        mService.getText(mNotificationText),
                        makeNotificationPendingIntent(type));
                notification.defaults = Notification.DEFAULT_ALL;
                break;
            case NotificationInteractionType.TYPE_ALERT_1101:
                notification = new Notification(
                        R.drawable.stat_download_info,
                        mService.getString(R.string.nia_info),
                        System.currentTimeMillis());
                notification.flags = Notification.FLAG_AUTO_CANCEL;
                notification.setLatestEventInfo(
                        mService,
                        mService.getText(R.string.app_name),
                        mService.getText(R.string.nia_info),
                        makeNotificationPendingIntent(type));
                notification.defaults = Notification.DEFAULT_ALL;
                break;
