...
public class NotesList extends ListActivity implements OnItemLongClickListener {
    ...
    protected void onDestroy() {
        ...
        if (mQueryHandler != null) {
            Log.d(TAG, "on Destroy deinit mQueryHandler");
            mQueryHandler.deinitQueryHandler();
        }
    }
    ...
    public void queryUpdateData() {
        ...
        if (noteadapter == null) {
            Log.d(TAG, "new a noteadapter");
        noteadapter = new NoteAdapter(this, null, 0);
        }
        if (mQueryHandler != null) {
            Log.d(TAG, "deinit mQueryHandler");
            mQueryHandler.deinitQueryHandler();
        }
        mQueryHandler = new QueryHandler(this.getContentResolver(), this);
        mQueryHandler.startQuery(0, mWaitCursorView, Notes.CONTENT_URI, Notes.PROJECTION,
