...
public class ModelImpl implements IChatManager {
            ...
            new AsyncTask<Void, Void, Void>() {
                ...
                protected Void doInBackground(Void... params) {
                    ...
                    try {
						...
						if(msg != null){
						 	Logger.w(TAG, "first message group" + msg.getTextMessage());
						}
						 else{
    ...
    public abstract class ChatImpl implements IChat, IRegistrationStatusListener,
     ...
     new ReceiveFileTransfer(fileTransferSession, isAutoAccept,isGroup);
        ...
        protected void reloadMessage(InstantMessage message, int messageType, int status, int messageTag) {
                ...
                try {
                    ...
                    if (null != messages) {
                        ...
                        for (int i = 0; i < size; i++) {
							...
							if(msg != null){
                            	Logger.w(TAG, "InstantMessage:" + "msg" + msg + "string" + msg.getTextMessage());	
							}
							else{
								Logger.w(TAG, "InstantMessage: is NULL");	
							}
                    ...
                    if (participantCount == 1) {
                        ...
                        if (chat == null) {
                           ...
                           ((GroupChat)chat).addGroupSubjectFromInvite(groupSubject);
    ...
    public boolean handleFileTransferInvitation(String sessionId, boolean isAutoAccept,boolean isGroup,String chatSessionId,String chatId) {
		Logger.w(TAG, "handleFileTransferInvitation isGroup" + isGroup + "isAutoAccept" + isAutoAccept);
        ...
        public static FileStruct from(IFileTransferSession fileTransferSession, String filePath)
			...
			if(!RcsSettings.getInstance().isFileTransferThumbnailSupported()){
				if(fileTransferSession.isHttpTransfer()){
					Logger.v(TAG, "Is HTTP transfer0");
            		fileStruct = new FileStruct(filePath, fileName, fileSize, sessionId, date, fileTransferSession.getRemoteFtContact());
					Logger.w(TAG, "fileContact"+fileTransferSession.getRemoteFtContact());
				}
				else{
            fileStruct = new FileStruct(filePath, fileName, fileSize, sessionId, date, fileTransferSession.getRemoteContact());
				}
			}
			else{
				Logger.v(TAG, "FileStruct thumbnail supported" + fileTransferSession.getFileThumbUrl());
				if(fileTransferSession.isHttpTransfer()){
					Logger.v(TAG, "Is HTTP transfer1");
            		fileStruct = new FileStruct(filePath, fileName, fileSize, sessionId, date,fileTransferSession.getRemoteFtContact(),fileTransferSession.getFileThumbUrl());
					Logger.w(TAG, "fileContact"+fileTransferSession.getRemoteFtContact());
				}
				else{
	...
	public void startGroupFileStransfer(){
		Logger.d(TAG, "startGroupFileStransfer()");
		if(groupFileTransferTarget != null){
			Logger.d(TAG, "startGroupFileStransfer() not empty");
			Object target = groupFileTransferTarget;
			int index = 0;
			IChat chat = null;
			Logger.d(TAG, "startGroupFileStransfer() size is: " + fileGroupTransferTag.size());
			if (target instanceof String) {
				Logger.d(TAG, "startGroupFileStransfer()1");
	            String contact = (String) target;
	            List<Participant> participants = new ArrayList<Participant>();
	            participants.add(new Participant(contact,contact));
	            chat = addChat(participants);
	        } else {
	        	Logger.d(TAG, "startGroupFileStransfer()2");
	            chat = getChat(target);
	        }
			for (String filePath : fileGroupTransferPath) {
				Object fileTransferTag = fileGroupTransferTag.get(index);
				Logger.d(TAG, "startGroupFileStransfer() Tag is: " + fileTransferTag + "Paths is: " + filePath);
				if(chat instanceof GroupChat)
	        	{
		        	List<ParticipantInfo> participantsInfo = ((GroupChat) chat).getParticipantInfos();
		        	List<String> participants = new ArrayList<String>();
		...
		Logger.d(TAG, "startGroupFileStransfer() exit");
	}
    ...
    public void handleGroupSendFileTransferInvitation(Object target, String filePath,
            Object fileTransferTag) {
        Logger.d(TAG, "handleGroupSendFileTransferInvitation() user starts to send file " + filePath
                + " to target " + target + ", fileTransferTag: " + fileTransferTag);
    ...
    public static class SentFileTransfer implements OnNetworkStatusChangedListerner {
        ...
        protected void send() {
			Logger.d(TAG, "checkNext() send new file");
            ApiManager instance = ApiManager.getInstance();
            if (instance != null) {
                MessagingApi messageApi = instance.getMessagingApi();
                if (messageApi != null) {
                    try {
                        ...
                        mFileTransferSession =
                            messageApi.transferFileGroup(mParticipants, mFileStruct.mFilePath, RcsSettings.getInstance().isFileTransferThumbnailSupported());
...
private void reloadGroupMessage(String tag, List<Integer> messageIds,
        ...
        for (ReloadMessageInfo messageInfo : messageList) {
            if (null != messageInfo) {
                ...
                ((ChatImpl) chat).reloadMessage(message, messageType, -1,-1);
            }
        }
    }
    }
            ...
            if (null != info) {
                ...
                if (obj instanceof InstantMessage) {
                    ...
                    if (!TextUtils.isEmpty(contact)) {
                        ...
                        ((ChatImpl) chat).reloadMessage(message, messageType, messageStatus, info.getMessageTag());
                    }
            ...
            if (null != info) {
                ...
                if (obj instanceof InstantMessage) {
                    ...
                    int messageStatus = info.getMessageStatus();
                        ...
                        ((GroupChat)chat).setmChatId(chatId);
                        ((ChatImpl) chat).reloadMessage(message, messageType, messageStatus, -1);
    ...
    private ReloadMessageInfo loadMessageFromId(TreeSet<Long> sessionIdList, Integer msgId,
        ...
        try {
            ...
            if (cursor.moveToFirst()) {
                ...
                int messagetag = cursor.getInt(cursor.getColumnIndex(RichMessagingData.KEY_ID));
                long sessionId =
                        cursor
                                .getLong(cursor
                ...
                if (EventsLogApi.TYPE_INCOMING_CHAT_MESSAGE == messageType
                    ...
                    fileInfo.setMessageTag(messagetag);
                    return fileInfo;
                }
                return null;
            } else {
                ...
                Logger.w(TAG, "loadMessageFronId() empty cursor, removing from mms DB");
                return null;
            }
        } finally {
            if (null != cursor) {
