...
public class TerminatingAdhocGroupChatSession extends GroupChatSession implements MsrpEventListener {
	...
	public TerminatingAdhocGroupChatSession(ImsService parent, SipRequest invite) {
	   ...
	   // Set alias name
		String fromAlias = ChatUtils.getFromAias(invite);
		if (logger.isActivated()) {
		    		logger.info("Fom alias name received is" + fromAlias);
		    	}

		String newFrom = fromAlias.split("\\ ")[0];
				if (logger.isActivated()) {
							logger.info("Fom alias name received is" + newFrom);
						}
				
        //Set Alias name
		setAliasName(newFrom);
