...
public class TerminatingHttpFileSharingSession extends HttpFileTransferSession implements HttpTransferEventListener {
    ...
    public TerminatingHttpFileSharingSession(ImsService parent, ChatSession chatSession,
            FileTransferHttpInfoDocument fileTransferInfo, String msgId,String from) {
		...
		setRemoteFtContact(from);
		if (logger.isActivated()) {
        		logger.info("TerminatingHttpFileSharingSession" + from);
        	}
		...
		if(getThumbnail() != null && thumbStreamForFile != null){
		try{
    ...
    public void run() {
        try {
            ...
            if (RcsSettings.getInstance().isFileTransferAutoAccepted() && mAutoAccept) {
