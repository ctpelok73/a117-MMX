...
public abstract class GroupChatSession extends ChatSession {
	...
	// Send message in CPIM
		...
		//from = "<" + from + ">";
		logger.info(" sendTextMessage" + from);
	...
	String content;
		if (useImdn) {
			...
			content = ChatUtils.buildCpimMessageWithDeliveredImdnAlias(from, to, imdnMsgId, StringUtils.encodeUTF8(txt), InstantMessage.MIME_TYPE);
		} else {
			// Send message in CPIM
	...
	boolean useImdn = getImdnManager().isImdnActivated();
		...
		if (useImdn) {
			// Send message in CPIM + IMDN delivered
            imdnMsgId = IdGenerator.getIdentifier();
			content = ChatUtils.buildCpimMessageWithDeliveredImdnAlias(from, to, imdnMsgId, geoDoc, GeolocInfoDocument.MIME_TYPE);
		} else {
			// Send message in CPIM
			content = ChatUtils.buildCpimMessageAlias(from, to, geoDoc, GeolocInfoDocument.MIME_TYPE);
		}
